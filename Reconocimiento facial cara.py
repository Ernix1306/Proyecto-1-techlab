...
cap = cv2.VideoCapture(0) # Abrir la webcam del sistema, cuando añadamos la webcam cambiar el numero 0 del parentesis por 1
...
while True: #   Bucle infinito 
    ...
    faces = face_cascade.detectMultiScale(gray, 1.1, 4) # Detectar todas las caras en la imagen, los parametros 1.1 y 4 se iran modificando en función de la imagen que pongamos para que funcones mejor
    for (x, y, w, h) in faces: # Dibujar un rectángulo alrededor de cada cara detectada
        cv2.rectangle(img, (x, y), (x+w, y+h), (0, 255, 0), 2)
    cv2.inshow('img', img) # Mostrar la cara con todas los modificaciones anteriores
